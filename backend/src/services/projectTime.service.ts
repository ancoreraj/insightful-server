import { ProjectTime } from '../models';

interface ProjectTimeFilters {
  start: number;
  end: number;
  timezone?: string;
  employeeId?: string;
...
class ProjectTimeService {
  ...
  async listProjectTime(organizationId: string, filters: ProjectTimeFilters): Promise<any[]> {
    ...
    // Execute query
    const projectTime = await ProjectTime.find(query)
      .sort({ start: -1 })
      .limit(limit)
      .lean();
...
export default new ProjectTimeService();
